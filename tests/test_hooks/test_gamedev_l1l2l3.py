"""L1/L2/L3 Tests: game_engine-gate-enforcer.py
...
HOOK_PATH = "D:/my_ai_projects/project_test_repos/game-dev/.claude/hooks/game_engine-gate-enforcer.py"


def make_valid_cwd():
    """Create a temp dir with valid session_state.json (anchor_ceremony present)."""
    tmp_dir = tempfile.mkdtemp(prefix="gamedev_test_")
...
def test_l3_anchor_ceremony_missing_blocks():
    ...
    tmp_dir = tempfile.mkdtemp(prefix="gamedev_l3_")
    state_dir = os.path.join(tmp_dir, ".claude", "state")
    os.makedirs(state_dir, exist_ok=True)
