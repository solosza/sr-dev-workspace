"""L1/L2/L3 Tests: ssh-gate-enforcer.py
...
HOOK_PATH = "D:/my_ai_projects/project_test_repos/isagawa-qa/platform-ssh/.claude/hooks/ssh-gate-enforcer.py"


def make_valid_cwd():
    """Create a temp dir with valid session_state.json (anchor_ceremony present)."""
    tmp_dir = tempfile.mkdtemp(prefix="ssh_test_")
...
def test_l1_shared_validators_import():
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         ...
         f"sys.path.insert(0, str(Path(r'{HOOK_PATH}').resolve().parents[5] / 'isagawa-kernel')); "
...
def test_l3_anchor_ceremony_missing_blocks():
    ...
    tmp_dir = tempfile.mkdtemp(prefix="ssh_l3_")
    state_dir = os.path.join(tmp_dir, ".claude", "state")
    os.makedirs(state_dir, exist_ok=True)
